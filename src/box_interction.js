...
export const parseTextract = (blocks, img_width = 1, img_height = 1) => {
  try {
    var textractResp = {
      Blocks: blocks || [],
    }
    ...
    for (let i = 0; i < respBlocks.length; i++) {
      var txt_obj = respBlocks[i]
      if (txt_obj["BlockType"] == "WORD") {
        var txt_poly = txt_obj["Geometry"]["Polygon"]
        var xy_poly = []
        for (let i = 0; i < txt_poly.length; i++) {
          var pt = txt_poly[i]
          xy_poly.push({
            x: txt_poly[i].X * img_width,
            y: txt_poly[i].Y * img_height,
          })
        }
        op_json.push({ Text: txt_obj["Text"], xy: xy_poly })
      }
    }
    return op_json
  } catch (err) {}
...
export const polyIntersection = (ip_poly, text_poly) => {
  try {
    let op_text = []
    for (let i = 0; i < text_poly.length; i++) {
      var result = intersect(ip_poly.xy, text_poly[i].xy)
      if (result.length > 0) {
        op_text.push(text_poly[i].Text)
      }
    }
    return op_text
  } catch (err) {
